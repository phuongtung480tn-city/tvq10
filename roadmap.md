...
- [x] 20/09: sửa lỗi chặn build — ô tối ưu ảnh trong Admin đọc sai nhánh cấu hình,
      2 lỗi kiểu dữ liệu ở behavior.ts, và khối `build` trong vite.config đặt sai
      chỗ nên bị bỏ qua (đã chuyển vào `vite:` và gỡ `manualChunks` vốn vô hiệu).
      Thêm script `typecheck`, `test`, `test:e2e`. Đã test submit thật: form gửi
      thành công → chuyển sang /cam-on, lead lưu, webhook + email vẫn chạy.
